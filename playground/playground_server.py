...
async def view_component(filename: str):
    ...
    # Prepare full component code for in-browser Babel
    import re
    processed = re.sub(r"^import\s+[^\n]*\n", "", jsx_content, flags=re.M)
    name_match = re.search(
        r"export\s+default\s+function\s+(\w+)\s*\(", processed)
    comp_name = name_match.group(1) if name_match else "GeneratedComponent"
    processed = re.sub(r"export\s+default\s+function\s+" +
                       comp_name, "function " + comp_name, processed)

    # Remove HTML comments which are not valid JSX
    jsx_body = re.sub(r'<!--.*?-->', '', jsx_body, flags=re.DOTALL)

    # Clean the JSX body by removing the closing parts
    jsx_body = jsx_body.strip()
    if jsx_body.endswith(');'):
        jsx_body = jsx_body[:-2].strip()
    if jsx_body.endswith('}'):
        jsx_body = jsx_body[:-1].strip()
...
{processed}
            ...
            try {{
                const RootComponent = {comp_name};
                root.render(<RootComponent />);
            }} catch (e) {{
                console.error('Render error:', e);
                const pre = document.createElement('pre');
                pre.textContent = 'Render error: ' + (e && e.message ? e.message : e);
...
{jsx_body}
    ...
    # Create JavaScript array from pages list
    pages_array = ", ".join(f'"{page}"' for page in pages)
...
""" + components_js + """
            ...
            try {
                ...
                function TemplateApp() {{
                    ...
                    const pageComponents = [""" + ", ".join(f"<{page.title()}Page />" for page in pages) + """];
